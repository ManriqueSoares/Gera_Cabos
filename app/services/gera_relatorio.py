...
def create_pdf(res: dict, img_path: str, aprovado: bool, msg_status: str) -> bytes:
    class PDF(FPDF):
        def footer(self):
            ...
            self.cell(
                0, 10, "Desenvolvido por: DELLAZARIG - Parte Ativa Gravataí", 0, 0, "C"
            )
    ...
    # Linha 1
    pdf.cell(
        95, 6, f"Luva: {res['nome_exibicao']} (Diam: {res['DIAM_LUVA']} mm)", border=0
    )
    pdf.cell(95, 6, f"Cabo: {res['cabo_selecionado']}", border=0, ln=True)

    # Linha 2
    pdf.cell(
        95, 6, f"Fio Retangular: {res['LARG_RECT']} x {res['ALT_RECT']} mm", border=0
    )
