...
# ==============================================================================
st.set_page_config(page_title="Luvas x Cabos", page_icon="logo.png", layout="wide", initial_sidebar_state="expanded")
...
lista_opcoes = ["Automático"] + [k for k in OPCOES_LUVA.keys() if k != "Personalizado"] + ["Personalizado"]
...
excluir_circular = st.sidebar.checkbox("Excluir Cabo Circular da Simulação", value=False)
...
if not excluir_circular:
    ...
    escolha_cabo = st.sidebar.selectbox("Seção Nominal do Cabo", list(OPCOES_CABO_CIRCULAR.keys()))
    
    if escolha_cabo == "Personalizado":
        DIAM_REDONDO_TOTAL = st.sidebar.number_input("Diâmetro Equivalente Total (mm)", value=10.0, step=0.5)
    else:
    ...
    st.sidebar.markdown("Diâmetro do Micro-Fio (Granularidade)", help="O cabo circular é multifilamentoso (composto por vários fios finos). O diâmetro selecionado será o diâmetro da 'partição' criada na simulação. Quanto menor, mais preciso o preenchimento, porém mais lenta a simulação.")
    col_mf1, col_mf2 = st.sidebar.columns([0.7, 1])
    with col_mf1:
        st.slider("", 0.4, 2.0, key="micro_slider", on_change=sync_micro_from_slider, step=0.1, label_visibility="collapsed")
    with col_mf2:
        st.number_input("", 0.4, 2.0, key="micro_number", on_change=sync_micro_from_number, step=0.1, format="%.2f", label_visibility="collapsed")
...
st.sidebar.markdown("Limite de Ocupação (%)", help="Taxa de ocupação é a porcentagem da área interna da luva que é preenchida pelos condutores. O padrão é 85%, conforme WPR-33156 ES (WEG México).")
col_oc1, col_oc2 = st.sidebar.columns([0.7, 1])
with col_oc1:
    st.slider("", 0.0, 100.0, key="ocupacao_slider", on_change=sync_ocupacao_from_slider, step=1.0, label_visibility="collapsed")
with col_oc2:
    st.number_input("", 0.0, 100.0, key="ocupacao_number", on_change=sync_ocupacao_from_number, step=1.0, format="%.2f", label_visibility="collapsed")
...
st.markdown("Este aplicativo simula a montagem prática de luvas, verificando fisicamente se o condutor cabe na luva. Além disso, realiza a simulação utilizando cabos retangulares e circulares para validar o encaixe em luvas curtas.")
...
def validar_rect(x, y, w, h, r_luva, rects):
    ...
    if (x**2+y**2 > r2) or ((x+w)**2+y**2 > r2) or \
       (x**2+(y+h)**2 > r2) or ((x+w)**2+(y+h)**2 > r2): return False
    for r in rects:
        if (x < r.x + r.w and x + w > r.x and y < r.y + r.h and y + h > r.y): return False
    return True

def ponto_livre(cx, cy, r_micro, r_luva, rects, micro_circs_existentes):
    if np.sqrt(cx**2 + cy**2) + r_micro > r_luva: return False
    for r in rects:
        closest_x = max(r.x, min(cx, r.x + r.w))
        closest_y = max(r.y, min(cy, r.y + r.h))
        dist_sq = (cx - closest_x)**2 + (cy - closest_y)**2
        if dist_sq < r_micro**2: return False 
    for c in micro_circs_existentes:
        if (cx - c.cx)**2 + (cy - c.cy)**2 < (2 * r_micro)**2: return False
    return True
...
if st.session_state.simulation_results:
    ...
    msg_pdf = mensagem if simulacao_aprovada else f"Reprovado. Motivos: {', '.join(motivos)}"
    try:
        pdf_bytes = create_pdf(res, tmp_img_path, simulacao_aprovada, msg_pdf)
        # Ajuste para horário de Brasília (UTC-3)
        fuso_br = datetime.timezone(datetime.timedelta(hours=-3))
        st.download_button(label="📄 Baixar Relatório (PDF)", data=pdf_bytes, file_name=f"Relatorio_Simulacao_{datetime.datetime.now(fuso_br).strftime('%Y%m%d_%H%M')}.pdf", mime="application/pdf", type="primary")
